...
class COVIDClassifier:
    def __init__(self, data_dir: str, args_model: str = "vgg19", output_dir: str = "outputs", img_size: int = 224,
                 batch_size: int = 32, epochs: int = 10, lr: float = 3e-4, weight_decay: float = 1e-4,
                 val_split: float = 0.15, test_split: float = 0.15, patience: int = 5, num_workers: int = 4):
        self.data_dir = data_dir
        self.output_dir = Path(output_dir) / "SARS-COV2-CT"
        ...
        self.args_model_name = args_model
    ...
    def _build_dataloaders(self):
        ...
        full_ds = ImageFolder(os.path.join(self.data_dir, "SARS-COV2-CT"), transform=train_tf)
        ...
        self.train_loader = DataLoader(train_ds, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers, pin_memory=True)
        self.val_loader = DataLoader(val_ds, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, pin_memory=True)
        self.test_loader = DataLoader(test_ds, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, pin_memory=True)
    ...
    def _build_model(self):
        ...
        #     nn.Dropout(0.5),
        #     nn.Linear(in_feats, len(self.class_names))
        # )
    ...
    def evaluate_model_metrics(self, targets, predicts, average='macro'):
        ...
        if len(self.class_names) == 2:      # Binary classification
            ...
            auc = roc_auc_score(targets, probs, multi_class='ovr', average=average)             # Multiclassification
    ...
    def run_lime_metrics(self, num_samples=8):
        ...
        outdir = self.output_dir / 'xai/lime_metrics'
        outdir.mkdir(parents=True, exist_ok=True)

        evaluator = XAIEvaluator(self.model, self.class_names, outdir)
    ...
    def run_shap_metrics(self, batch_size=8):
        ...
        outdir = self.output_dir / 'xai/shap_metrics'
        outdir.mkdir(parents=True, exist_ok=True)
    ...
    def run_pebex_metrics(self):
        ...
        # # lấy batch mẫu
        # sample_imgs, labels = next(iter(self.test_loader))
        # sample_imgs, labels = sample_imgs[:batch_size], labels[:batch_size]
