...
class XAIEvaluator:
    ...
    def evaluate_with_shap(self, loader, batch_size: int = 8, noise_sigma: float = 0.02, large_mask_prob: float = 0.2):
        ...
        #imgs = imgs[:batch_size].to(device)    # (N, C, H, W)
        imgs = imgs.to(device)
        N = imgs.shape[0]
        ...
        for i in range(N):
            p = int(y_model[i])
            arr = shap_noisy_per_class[p][i]   # (C,H,W)
            heatmaps_noisy.append(np.abs(arr).sum(axis=0))
        vecs_noisy = [self.vectorize_explanation(h) for h in heatmaps_noisy]
        stability_val = float(np.mean([np.linalg.norm(a - b) for a, b in zip(expl_vecs, vecs_noisy)]))
        ...
        raw_shap_largep = explainer.shap_values(imgs_largep)
        shap_largep_per_class = to_shap_per_class_array(raw_shap_largep)
        heatmaps_largep = []
        for i in range(N):
            p = int(y_model[i])
            arr = shap_largep_per_class[p][i]
            heatmaps_largep.append(np.abs(arr).sum(axis=0))
        vecs_largep = [self.vectorize_explanation(h) for h in heatmaps_largep]
        robustness_val = float(np.mean([np.linalg.norm(a - b) for a, b in zip(expl_vecs, vecs_largep)]))
    ...
    # --------- Custom Explainer (cho MFPP-PeBEx) ----------
    def evaluate_with_custom_explainer(self, loader, explainer, explainer_name="Custom"):
        """
        Đánh giá với custom explainer (như MFPP-PeBEx)
        
        Args:
            loader: DataLoader
            explainer: explainer object có method explain_one(img_tensor, mode)
            explainer_name: tên explainer để đặt tên metrics
        """
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        all_expl_vecs, all_used_feats = [], []
        all_y_model, all_y_local = [], []
        ...
        for batch_imgs, batch_labels in loader:
            ...
            for i in range(len(imgs)):
                # explain_one trả về (heatmap, predicted_label)
                heat, pred_from_importance = explainer.explain_one(imgs[i].to(device), "black")
        ...
        results = {
            f"{explainer_name}_Fidelity": self.fidelity(all_y_model, all_y_local),
            f"{explainer_name}_Comprehensibility": mu_comp,
            f"{explainer_name}_Consistency": mu_cons,
            f"{explainer_name}_LocalAcc": self.local_accuracy(all_y_model, all_y_local),
            f"{explainer_name}_Stability": self.stability(all_expl_vecs),
            f"{explainer_name}_Similarity": mu_cons,
            f"{explainer_name}_Robustness": self.robustness(all_expl_vecs),
            f"{explainer_name}_Time": exp_time,
        }
        results.update(self.fcc_score(results[f"{explainer_name}_Fidelity"], 
                                    results[f"{explainer_name}_Comprehensibility"], 
                                    results[f"{explainer_name}_Consistency"]))
        return results